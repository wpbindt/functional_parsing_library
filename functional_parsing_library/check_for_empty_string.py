...
def check_for_empty_string(
    parse_function: Callable[[str], ParseResults[str, U] | CouldNotParse],
) -> Callable[[str], ParseResults[str, U] | CouldNotParse]:
    @functools.wraps(parse_function)
    def parser_(to_parse: str) -> ParseResults[str, U] | CouldNotParse:
        if len(to_parse) == 0:
